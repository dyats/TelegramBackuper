...
string botToken = appConfig["Telegram:Bot:Token"]!;
...
var channelNames = appConfig.GetSection("Telegram:Channels")
    .GetChildren()
    .Select(x => x.Value)
    .ToList();

// Bot
var tgBotOptions = new TelegramBotClientOptions(token: botToken);
TelegramBotClient bot = new TelegramBotClient(tgBotOptions);
...
IChannelDistributorService channelDistributorService = new ChannelDistributorService(tdlibService);
...
foreach (var channel in channels.Where(x => x.Title != ""))
{
    // Twitter Api
    ...
    foreach (var message in messages)
    {
        ...
        //}
        if (message.message.Contains("twitter") && channel.Title != "")
        {
            var link = Regex.Match(message.message, RegexCondition.Link).Value;
            twitterLinks.Add(link);

            await bot.SendTextMessageAsync(channel.ID, link);
            //await channelDistributorService.DistributeContentAsync(channel, link);
        }
